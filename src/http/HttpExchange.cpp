...
fiber::async::Task<common::IoResult<ReadBodyResult>> HttpExchange::read_body(void *buf, size_t len) noexcept {
    ...
    if (!request_content_length_set_) {
        request_body_done_ = true;
        out.end = true;
        co_return out;
    }
    ...
    size_t copied = 0;
    if (want == 0) {
        out.end = request_body_read_ >= request_content_length_;
        co_return out;
    }
...
fiber::async::Task<common::IoResult<void>> HttpExchange::discard_body() noexcept {
    std::array<char, kBodyReadChunk> buf{};
    for (;;) {
...
fiber::async::Task<common::IoResult<void>> HttpExchange::send_response_header(int status, std::string_view reason) {
    ...
    bool close_conn = response_close_;
    if (!close_conn) {
        if (version_ == HttpVersion::HTTP_1_0) {
            close_conn = !request_keep_alive_;
        } else {
            close_conn = request_close_;
        }
    }

    if (!response_content_length_set_ && !response_chunked_) {
        response_content_length_set_ = true;
        response_content_length_ = 0;
    }

    std::string header;
    header.reserve(128 + response_headers_.size() * 32);
    if (version_ == HttpVersion::HTTP_1_0) {
    ...
    if (response_content_length_set_ && !response_headers_.contains("Content-Length")) {
        header.append("Content-Length: ");
        header.append(std::to_string(response_content_length_));
        header.append("\r\n");
    }
    if (response_chunked_ && !response_headers_.contains("Transfer-Encoding")) {
        header.append("Transfer-Encoding: chunked\r\n");
    }
    if (close_conn && !response_headers_.contains("Connection")) {
...
fiber::async::Task<common::IoResult<size_t>> HttpExchange::write_body(const uint8_t *buf, size_t len, bool end) noexcept {
    ...
    if (response_content_length_set_) {
    ...
    response_body_sent_ += len;
    if (end) {
        if (response_content_length_set_ && response_body_sent_ != response_content_length_) {
            co_return std::unexpected(common::IoErr::Invalid);
        }
        response_complete_ = true;
        if (!response_content_length_set_ && !response_chunked_) {
